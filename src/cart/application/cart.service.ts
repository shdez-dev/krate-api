...
export class CartService {
  ...
  async addItem(userId: string, dto: AddItemDto): Promise<CartItem> {
    ...
    if (dto.quantity < 1) throw new BadRequestException('La cantidad debe ser al menos 1');
  ...
  async updateItem(userId: string, itemId: string, quantity: number): Promise<CartItem> {
    ...
    if (!cart || item.cartId !== cart.id) throw new ForbiddenException('Sin acceso a este ítem');
