...
describe('OrdersService', () => {
  ...
  describe('findById', () => {
    ...
    it('lanza NotFoundException si no existe', async () => {
      ...
      await expect(
        service.findById('order-x', 'user-1', UserRole.CUSTOMER),
    ...
    it('lanza ForbiddenException si el usuario no es el dueño', async () => {
      orderRepo.findById.mockResolvedValue(mockOrder);
      await expect(
        service.findById('order-1', 'other-user', UserRole.CUSTOMER),
  ...
  describe('create', () => {
    it('lanza BadRequestException si el carrito está vacío', async () => {
      ...
      await expect(
        service.create('user-1', { shippingAddress }),
  ...
  describe('updateStatus', () => {
    ...
    it('lanza NotFoundException si no existe', async () => {
      orderRepo.findById.mockResolvedValue(null);
      await expect(
        service.updateStatus('order-x', OrderStatus.PAID),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
